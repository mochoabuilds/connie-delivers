...
class SignUpController: UIViewController {
    ...
    private let accountTypeSegmentedControl: UISegmentedControl = {
        let sc = UISegmentedControl(items: ["Guest", "Store"])
    ...
    @objc func handleSignUp(){
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (result, error) in
            if let error = error {
                print("FIX ME: Failed to register user with error \(error.localizedDescription)")
                return
            }
            ...
            // Creates Person Node with Unique UID and Dictionary of Values About Person
            Database.database().reference().child("users").child(uid).updateChildValues(values,
                    withCompletionBlock: { (error, ref) in
                        
                    // Once Login Succesful, Call Function to Configure UI
